...
export const ARCADE_GAME_RULES: Record<string, GameRuleDetail> = {
  ...
  carrom: {
    ...
    howToPlay: [
      ...
      "4. 15-SECOND CONSULTATION PROTOCOL: Teammates may speak for the first 5 seconds of the turn. Once the shooter sets their hand/striker on the baseline, all speech MUST cease. Pointing at pieces or coaching while set incurs a 1 Due foul penalty.",
      "5. SPECIALIZED DOUBLES BREAKS: (A) North-Feed Spread (70-80% power on right apex to bias White pieces to Partner A2's quadrant). (B) Defensive Cluster Lock (30-40% soft strike leaving cluster closed to force Opponent B1 into a tough open). (C) Queen-Release Wedge (splits front gap to expose Queen for partner tag-team cover).",
      "6. TEAMMATE FEEDING & DECISION FRAMEWORK: Inning priority: (1) 100% clean pot -> ATTACK. (2) Safe partner feed -> Baseline Push / Pocket Lip Rest. (3) Else -> FREEZE / CHOKE (lock opponent piece flat against next shooter's rail with 0mm gap).",
      "7. WHITE SLAM & BLACK SLAM (INNING 1 RUNOUT): White Slam = Breaker clears all 9 White + Queen in Turn 1 (+12 pts). Black Slam = Non-breaker clears all 9 Black + Queen in Turn 1 (+12 pts).",
      "8. QUEEN COVERING & 22-PT CAP: Queen must be covered by pocketing own color (or any piece in Freestyle) on immediate next stroke. In ICF play, once a player reaches 22+ pts, the 3-pt Queen bonus is zeroed out.",
      "9. FOULS & DUE PENALTIES: Pocketing striker (scratch) ends turn + 1 piece returned to center as Due penalty. Pocketing striker + own piece = 2 pieces returned."
    ],
