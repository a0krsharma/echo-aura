...
export const ARCADE_GAME_RULES: Record<string, GameRuleDetail> = {
  ...
  carrom: {
    ...
    howToPlay: [
      ...
      "3. BANK SHOT PHYSICS & CUSHION RESTITUTION: Wooden rails have restitution e ≈ 0.72-0.84. Rebound angle tan(θ_r) = (tan(θ_i) - μ_k(1+e))/e. Hard strikes compress wood micro-fibers, steepening the exit path. Running spin widens rebounds; check spin steepens rebound.",
      "4. 15-SECOND SHOT CLOCK BIOMECHANICS: (0-4s) Tactical scan & partner nod. (4-8s) Hand placement & base anchor. (8-12s) Grip tendon tension loading. (12-14s) Release & follow-through. Zero speech once set; pointing is a 1 Due penalty.",
      "5. SPECIALIZED DOUBLES BREAKS: (A) North-Feed Spread (70-80% power on right apex to bias White pieces to Partner A2's quadrant). (B) Defensive Cluster Lock (30-40% soft strike leaving cluster closed). (C) Queen-Release Wedge (splits front gap for tag-team cover).",
      "6. LATE-GAME QUEEN STANDOFFS (LAW 22): (A) 1-Piece Lockout: Cannot pot final piece before Queen (foul). (B) Simultaneous Double-Pot: Sinking Queen + final piece on same stroke wins board instantly. (C) Hostage Burial: Hiding Queen behind opponent cluster to stall their runout.",
      "7. TEAMMATE FEEDING & DECISION FRAMEWORK: Inning priority: (1) 100% clean pot -> ATTACK. (2) Safe partner feed -> Baseline Push / Pocket Lip Rest. (3) Else -> FREEZE / CHOKE (lock opponent piece flat against next shooter's rail with 0mm gap).",
      "8. WHITE SLAM & BLACK SLAM (INNING 1 RUNOUT): White Slam = Breaker clears all 9 White + Queen in Turn 1 (+12 pts). Black Slam = Non-breaker clears all 9 Black + Queen in Turn 1 (+12 pts).",
      "9. QUEEN COVERING & 22-PT CAP: Queen must be covered by pocketing own color (or any piece in Freestyle) on immediate next stroke. In ICF play, once a player reaches 22+ pts, the 3-pt Queen bonus is zeroed out.",
      "10. FOULS & DUE PENALTIES: Pocketing striker (scratch) ends turn + 1 piece returned to center as Due penalty. Pocketing striker + own piece = 2 pieces returned."
    ],
