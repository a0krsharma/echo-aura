...
export default function MendicotGame({ match, currentUid }: MendicotGameProps) {
  ...
  useEffect(() => {
    if (!ms || match.status !== "PLAYING" || match.mode !== "VS_COMPUTER") return;
    const botPlayer = Object.values(match.players || {}).find(
      (p) => p.uid === ms.currentTurnUid && p.isBot
    );
    if (botPlayer) {
      executeMendicotBotTurn(match);
    }
  }, [match, ms?.currentTurnUid]);

  if (!ms) return <div className="text-white font-mono p-4">Loading Mendicot Table...</div>;

  const hands: Record<string, string[]> = JSON.parse(ms.handsStr || "{}");
  const myHand = hands[currentUid] || [];
  const isMyTurn = ms.currentTurnUid === currentUid && match.status === "PLAYING";

  const handlePlayCard = async (card: string) => {
    if (!isMyTurn || match.status === "FINISHED") return;
    soundSynth.playSnare();
    try {
      await playMendicotCard(match.id, currentUid, card);
    } catch (e) {
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-black border-2 border-white p-3 sm:p-5 font-mono text-white space-y-4 select-none shadow-[0_0_40px_rgba(255,255,255,0.15)] relative">
      ...
      <div className="flex items-center justify-between border-b-2 border-white pb-2 text-[10px] sm:text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="font-black uppercase tracking-widest text-white">
            // MENDICOT / DEHLA PAKAD [ 2V2 PARTNERSHIP ]
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      {/* 10s Tally Counter */}
      <div className="grid grid-cols-2 gap-3 bg-neutral-950 p-3 border border-neutral-800 rounded text-center">
        <div className="border border-neutral-800 p-2 rounded">
          <span className="text-[10px] text-neutral-400 font-bold block uppercase">TEAM 1 (YOU + P3)</span>
          <span className="text-xl font-black text-white font-mono">{ms.team1TensCount} / 4 TENS</span>
        </div>
        <div className="border border-neutral-800 p-2 rounded">
          <span className="text-[10px] text-neutral-400 font-bold block uppercase">TEAM 2 (P2 + P4)</span>
          <span className="text-xl font-black text-white font-mono">{ms.team2TensCount} / 4 TENS</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs text-neutral-400">
          ...
          <span className="text-[10px]">CAPTURE 10s TO WIN WHITEWASH (KOT)</span>
        </div>
            ...
            return (
              <button
                ...
                className={`w-10 h-16 sm:w-11 sm:h-18 rounded border-2 flex flex-col items-center justify-between p-1 font-black transition-all cursor-pointer ${
                  ...
                  isTen ? "ring-2 ring-emerald-400 border-emerald-400" : ""
                } ${
      ...
      {ms.lastActionLog && (
        <div className="border border-neutral-800 bg-neutral-950 px-3 py-2 text-xs text-white flex items-center gap-2">
          ...
          <span className="truncate uppercase font-black">{ms.lastActionLog}</span>
        </div>
      )}
      ...
      {match.status === "FINISHED" && (
        <div className="border-4 border-white bg-black p-6 text-center space-y-2 animate-bounce shadow-2xl">
          ...
          <h2 className="font-black text-base uppercase tracking-widest text-white">
            🏆 {match.winnerHandle} CAPTURED THE 10s & WON!
          </h2>
