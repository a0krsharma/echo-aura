...
export default function UnoGame({ match, currentUid }: UnoGameProps) {
  ...
  }, [match, us?.currentTurnUid, us?.discardTop?.color, us?.discardTop?.value]);
  ...
  const myHand = useMemo(() => {
    ...
    if (sortBy === "COLOR") {
      ...
      sorted.sort((a, b) => (colorOrder[a.color] || 0) - (colorOrder[b.color] || 0) || a.value.localeCompare(b.value));
    } else {
  ...
  const handleCardClick = async (card: UnoCard) => {
    ...
    try {
      ...
      const result = await playUnoCard(match.id, currentUid, card, undefined, willHaveOneCard || hasCalledUno);
  ...
  const handleSelectWildColor = async (chosenColor: "RED" | "BLUE" | "GREEN" | "YELLOW") => {
    ...
    try {
      const willHaveOneCard = rawHand.length === 2;
      const result = await playUnoCard(match.id, currentUid, card, chosenColor, willHaveOneCard || hasCalledUno);
      if (result.won) soundSynth.playFanfare();
      setHasCalledUno(false);
    } catch (e) {
      soundSynth.playBuzzer();
    ...
    <div className="w-full max-w-xl mx-auto bg-black border-2 border-white p-3 sm:p-5 font-mono text-white space-y-4 select-none shadow-[0_0_40px_rgba(255,255,255,0.15)] relative">
      ...
      <div className="border-2 border-white bg-neutral-950 p-6 flex items-center justify-center gap-8 rounded-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center gap-2">
          <span className="text-[9px] text-neutral-400 font-bold uppercase tracking-wider">DRAW DECK</span>
          <button
            type="button"
          ...
          >
            ...
            <div className="text-[9px] uppercase font-black self-end">DRAW +1</div>
          </button>
        </div>
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            {/* Top-left Value */}
            <span className={`text-xs font-black self-start leading-none ${COLOR_MAP[us.discardTop.color]?.text}`}>
              {us.discardTop.value}
            </span>
            ...
            {/* Bottom-right Value */}
            <span className={`text-xs font-black self-end leading-none ${COLOR_MAP[us.discardTop.color]?.text}`}>
              {us.discardTop.value}
            </span>
          </div>
        </div>
