 ...
 */
export default function OrbitLogo({ className = "" }: { className?: string }) {
  return (
    <Link href="/" className={`inline-flex items-center gap-3.5 group select-none ${className}`}>
      {/* Planetary Orbit Visualizer */}
      <div className="relative w-9 h-9 flex items-center justify-center shrink-0">
        {/* Sun in center */}
        <div className="w-3.5 h-3.5 rounded-full bg-gradient-to-tr from-amber-500 via-yellow-400 to-amber-200 shadow-[0_0_12px_rgba(251,191,36,0.9)] animate-pulse z-10" />
        ...
        <div className="absolute inset-0 animate-[spin_4s_linear_infinite]">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-gradient-to-br from-cyan-400 via-blue-500 to-indigo-600 shadow-[0_0_8px_rgba(34,211,238,0.9)]" />
        </div>
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-1.5">
          <span className="font-serif italic text-2xl font-bold tracking-tight text-white group-hover:text-amber-300 transition-colors">
            Echo.
          </span>
